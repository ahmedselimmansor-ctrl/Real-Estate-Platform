...
Analytics, documents and the money maths for the TopChoice.
...
| Auth | CONTRACT §5 — HS256 access token verified locally against `JWT_ACCESS_SECRET`, checking `iss=topchoice-api` / `aud=topchoice-clients` |
...
**slug** or its **reference number** (`TC-1042`).
...
# --- brochure -------------------------------------------------------------
curl -sk -OJ "$BASE/property/TC-1042/brochure.pdf"      # -> topchoice-<slug>-tc-1042.pdf
curl -skI "$BASE/property/TC-1042/brochure.pdf" | grep -i -e content-disposition -e x-cache
...
* `Content-Type: application/pdf`, `Content-Disposition: attachment; filename="topchoice-<slug>-<reference>.pdf"`
...
| `JWT_ACCESS_SECRET`, `JWT_ISSUER`, `JWT_AUDIENCE` | — / `topchoice-api` / `topchoice-clients` | token verification |
